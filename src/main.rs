...
fn main() {
    let database_path: String = "database".to_string();
    ...
    if !fs::metadata(database_path.clone()).is_ok() {
        ...
        let _ = fs::write(database_path.clone() + "/key.txt", key.as_bytes());

        println!("Generating initialization vector");
        let init_vec = startup::generate_key(); 
        let _ = fs::write(database_path.clone() + "/IV.txt", init_vec.as_bytes());
        ...
        let _ = fs::write(database_path.clone() + "/logins.txt", encrypted_text.as_bytes());

        let mut inputted_password = "".to_string();
        ...
        let _ = fs::write(database_path.clone() + "/password.txt", hashed_password.as_bytes());
